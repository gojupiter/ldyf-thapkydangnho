...
} elseif ( isset( $_SESSION['tc_cart_subtotal']) && isset($_SESSION['tc_discount_code'] ) ) {
    $discount->discounted_cart_total($_SESSION['tc_cart_subtotal'], $_SESSION['tc_discount_code']);

} elseif ( isset( $_SESSION['tc_discount_code'] ) && $_SESSION['tc_discount_code'] ) {
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="actions" colspan="<?php echo apply_filters('tc_cart_table_colspan', '5'); ?>">
                                    ...
                                    <?php } ?>
                                    <input type="submit" id="update_cart" value="<?php _e("Update Cart", "tc"); ?>" class="tickera_update tickera-button"><input type="submit" id="empty_cart" value="<?php _e("Empty Cart", "tc"); ?>" class="tickera_update tickera-button">
                    ...
                    foreach ($buyer_form_fields as $field) {
                        ...
                        ?><?php if ($field['field_type'] == 'text') { ?><div class="fields-wrap <?php
                            ...
                            ?>"><label><span><?php echo ($field['required'] ? '*' : ''); ?><?php echo $field['field_title']; ?></span><input type="<?php echo $field['field_type']; ?>" <?php
                                ...
                                <!-- Buyer confirm email field add on cart page start -->
                                         
                                <?php if ($field['field_type'] == 'email') {
                                if ((isset($tc_general_settings['email_verification_buyer_owner']) && $tc_general_settings['email_verification_buyer_owner'] == 'yes' && $field['field_name'] == 'confirm_email' ) || $field['field_name'] !== 'confirm_email'  ) {
                                   ?>
                               <div class="fields-wrap <?php
                                 ...
                                 if (isset($field['validation_type'])) {
                                     $validation_class = 'tc_validate_field_type_confirm_' . $field['validation_type'];
                                 } else {
                                     $validation_class = '';
                                 }
                                 ?>"><label><span><?php echo ($field['required'] ? '*' : ''); ?><?php echo $field['field_title']; ?></span><input type="<?php echo $field['field_type']; ?>" <?php
                                  ...
                                  <!-- Buyer confirm email field add on cart page end -->
                        ...
                        for ($i = 1; $i <= $ordered_count; $i++) {
                            ...
                            <div class="owner-info-wrap">
                                <?php foreach ($owner_form_fields as $field) { ?>
								  ...
								  <!-- Attendee confirm email field add on cart page start -->

								   <?php if ($field['field_type'] == 'email') { ?>
								   <?php
									if ((isset($tc_general_settings['email_verification_buyer_owner']) && (isset($tc_general_settings['show_owner_email_field'])) && $tc_general_settings['email_verification_buyer_owner'] == 'yes' && $tc_general_settings['show_owner_email_field'] == 'yes' && ($field['field_name'] == 'owner_confirm_email' ) || $field['field_name'] !== 'owner_confirm_email')  ) {
									   ?>
									   <div class="fields-wrap <?php
									   if (isset($field['field_class'])) {
										   echo $field['field_class'];
									   ...
									   ?>"><label><span><?php echo ($field['required'] ? '*' : ''); ?><?php echo $field['field_title']; ?></span><input type="<?php echo $field['field_type']; ?>" <?php
                                        ...
                                        <!-- Attendee confirm email field add on cart page end -->
