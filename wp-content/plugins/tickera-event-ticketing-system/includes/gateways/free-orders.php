...
class TC_Gateway_Free_Orders extends TC_Gateway_API {
    ...
    function order_confirmation_message($order, $cart_info = '') {
        ...
        switch ( $order->details->post_status ) {
            ...
            case 'order_refunded':
                $content .= '<p>' . sprintf(__('Your payment via %s for this order totaling <strong>%s</strong> is refunded.', 'tc'), $this->public_name, apply_filters('tc_cart_currency_and_format', $order->details->tc_payment_info['total'])) . '</p>';
                break;
