      ...
      <?php 
        ...
        function cart_payment( $echo = false )
        {
            ...
            if ( isset( $_SESSION['discounted_total'] ) && isset( $_SESSION['tc_cart_total'] ) && round( $_SESSION['discounted_total'], 2 ) == round( $_SESSION['tc_cart_total'], 2 ) ) {
                // Logic to continue to process free tickets
            } else {
                $_SESSION['tc_gateway_error'] = sprintf( __( 'Sorry, something went wrong. %sPlease try again%s.', 'tc' ), '<a href="' . $this->get_cart_slug( true ) . '">', '</a>' );
            }
        ...
        function tc_checkout_payment_form( $content, $cart )
        {
            ...
            foreach ( (array) $tc_gateway_plugins as $code => $plugin ) {
                ...
                if ( $this->gateway_is_network_allowed( $code ) ) {
                    $gateway = ( 0 == $cart_total ? new $plugin() : new $plugin[0]() );
                    ...
                    if ( $visible ) {
                        $tickera_max_height = ( 1 == count( (array) $tc_gateway_active_plugins ) ? 'tickera-height' : '' );
                        ...
                        $content .= ( 1 == count( (array) $tc_gateway_active_plugins ) ? '<input type="radio" class="tc_choose_gateway tickera-hide-button" id="' . $gateway->plugin_name . '" name="tc_choose_gateway" value="' . $gateway->plugin_name . '" checked ' . checked( ( isset( $_SESSION['tc_payment_method'] ) ? $_SESSION['tc_payment_method'] : '' ), $gateway->plugin_name, false ) . '/>' : '<input type="radio" class="tc_choose_gateway" id="' . $gateway->plugin_name . '" name="tc_choose_gateway" value="' . $gateway->plugin_name . '" ' . checked( ( isset( $_SESSION['tc_payment_method'] ) ? $_SESSION['tc_payment_method'] : '' ), $gateway->plugin_name, false ) . '/>' );
